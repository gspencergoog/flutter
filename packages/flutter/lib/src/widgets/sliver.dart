...
abstract class _SliverChildHandlingDelegate {
  ...
  const _SliverChildHandlingDelegate();
...
/// case of [SliverChildListDelegate]/[SliverAnimatedChildListDelegate]) or
/// lazily provided ones (such as in the case of
/// [SliverChildBuilderDelegate]/[SliverAnimatedChildBuilderDelegate]).
///
/// ### Destruction
///
...
///
/// * Moving the ownership of non-trivial UI-state-driving business logic out of
///   the sliver child subtree. For instance, if a list contains posts with
///   their number of upvotes coming from a cached network response, store the
///   list of posts and upvote number in a data model outside the list. Let the
///   sliver child UI subtree be easily recreate-able from the source-of-truth
///   model object. Use [StatefulWidget]s in the child widget subtree to store
...
///   render object is scrolled out of view, the sliver keeps the child's render
...
///
///   This only works if subclasses don't wrap the child widget subtree with
///   other widgets such as [AutomaticKeepAlive] and [RepaintBoundary] via
///   `addAutomaticKeepAlives` and `addRepaintBoundaries`.
///
/// * Using [AutomaticKeepAlive] widgets (inserted by default by many
///   subclasses) [AutomaticKeepAlive] allows descendant widgets to control
///   whether the subtree is actually kept alive or not. This behavior is in
///   contrast with [KeepAlive], which will unconditionally keep the subtree
///   alive.
///
...
///   of children.
abstract class SliverChildDelegate extends _SliverChildHandlingDelegate {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
...
/// children than are visible through the [Viewport]. Rather than receiving
/// their children as an explicit [List], they receive their children using a
/// [SliverAnimatedChildDelegate].
///
/// It's uncommon to subclass [SliverAnimatedChildDelegate]. Instead, consider
/// using one of the existing subclasses that provide adaptors to builder
/// callbacks or explicit child lists.
///
/// {@macro flutter.widgets.SliverChildDelegate.lifecycle}
///
/// See also:
///
/// * [SliverAnimatedChildBuilderDelegate], which is a delegate that uses a
///   builder callback to construct the children.
/// * [SliverAnimatedChildListDelegate], which is a delegate that has an
///   explicit list of children.
abstract class SliverAnimatedChildDelegate extends _SliverChildHandlingDelegate {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const SliverAnimatedChildDelegate();

  /// Returns the child with the given `index`, animated by the given
  /// [Animation].
  ///
  /// Should return null if asked to build a widget with a greater index than
  /// exists. If this returns null, [estimatedChildCount] must subsequently
  /// return a precise non-null value (which is then used to implement
  /// [RenderSliverBoxChildManager.childCount]).
  ///
  /// Subclasses typically override this function and wrap their children in
  /// [AutomaticKeepAlive], [IndexedSemantics], and [RepaintBoundary] widgets.
  ///
...
abstract class _SliverChildHandlingBuilderDelegate<T extends Function> extends _SliverChildHandlingDelegate {
  ...
  const _SliverChildHandlingBuilderDelegate(
    this.builder, {
...
/// Many slivers lazily construct their box children to avoid creating more
/// children than are visible through the [Viewport]. This delegate provides
/// children using a [NullableAnimatedIndexedWidgetBuilder] callback, so that
/// the children do not even have to be built until they are displayed.
///
/// The widgets returned from the builder callback are automatically wrapped in
/// [AutomaticKeepAlive] widgets if [addAutomaticKeepAlives] is true (the
/// default) and in [RepaintBoundary] widgets if [addRepaintBoundaries] is true
...
/// {@end-tool}
///
/// In certain cases, only a subset of child widgets should be annotated with a
/// semantic index. For example, in [ListView.separated()] the separators do not
/// have an index associated with them. This is done by providing a
...
///
/// This sample code shows how to use `semanticIndexCallback` to handle
/// annotating a subset of child nodes with a semantic index. There is a
/// [Spacer] widget at odd indexes which should not have a semantic index.
///
/// ```dart
/// CustomScrollView(
///   semanticChildCount: 5,
...
class SliverAnimatedChildBuilderDelegate extends _SliverChildHandlingBuilderDelegate<AnimatedItemBuilder> implements SliverAnimatedChildDelegate {
  ...
  /// new index for a child that was previously located at a different index to
  /// attach the existing state to the [Widget] at its new location.
  const SliverAnimatedChildBuilderDelegate(
    super.builder, {
    super.findChildIndexCallback,
    super.childCount,
    super.addAutomaticKeepAlives = true,
  ...
  Widget? build(BuildContext context, int index, Animation<double> animation) {
    ...
    Widget? child;
    try {
      child = builder(context, index, animation);
    } catch (exception, stackTrace) {
      child = _createErrorWidget(exception, stackTrace);
    }
    if (child == null) {
